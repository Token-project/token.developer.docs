...
You can also use this method to test whether FundingBox API authentication is functional. 
Method: GET
>```
>
>```
...
The response is JSON-encoded object containing the ID and the time of the stamp. The receipt object gives an estimated time in seconds for the different receipt types (ethereum and bitcoin) to be ready for retrieval.
Method: POST
>```
...
The response is a JSON-encoded object containing an array of stamps. If a certain receipt found inside a stamp is a number instead of an object, that number is the estimated number of seconds before the final receipt will be ready.
Method: GET
>```
